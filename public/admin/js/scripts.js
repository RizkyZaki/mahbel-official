    ...
    NioApp.CurrentLink = function () {
        ...
        $(_link).each(function () {
            var self = $(this),
                ...
                isCurrentPage = fileName === _self_link; // Check if the link matches the current page
                ...
                $self_id
                    .steps({
                        ...
                        onFinished: function onFinished(event, currentIndex) {
                            var $finishButton = $self.find('.actions a[href="#finish"]');
                            $finishButton.text('Processing...');
                            $finishButton.addClass('disabled').off('click');
                            $self_id.submit();
                        },
                    })
                // .validate({
    ...
    NioApp.DataTable = function (elm, opt) {
        if ($(elm).exists()) {
            $(elm).each(function () {
                ...
                var button = hasExport
                    ? '<"dt-export-buttons d-flex align-center' +
                ...
                var def = {
                    ...
                    createdRow: function (row, data, _index) {
                        ...
                        const length = $("td", row).length;

                        $.each($("td", row), function (index) {
                            const column = DT_COLUMNS[index];
                            if (!column || !("data" in column)) return;

                            $(this).attr("data-column", column.data);
                            $(this).addClass(
                                "nk-tb-col " + (column ? column.rowClass : "")
                            );
                        });
                    },
                };
